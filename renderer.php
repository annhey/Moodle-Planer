...
// Added include
include($CFG->dirroot.'/mod/scheduler/studentcomment_form.php');
...
class mod_scheduler_renderer extends plugin_renderer_base {
    ...
    /**
     * Renders a table of slots that are available to the student for booking
     *
    ...
    public function render_scheduler_slot_booker(scheduler_slot_booker $booker) { 
        ...
        /****************** Modal window with form for student comments and file upload ************************/
        
        // CSS for message window
        echo '<link rel="stylesheet" href="css\student_comment.css">';
        
        // Settings for the message form
        $context = $this->page->context;
        $contextid = $context->id;
        $component= 'mod_scheduler';
        $filearea = 'attachment';
        $itemid = 0;       //extra line: list($context, $course, $cm) = get_context_info_array($contextid);
        ...
        // Initialise mform and pass on $data-object (which at this point contains only the scheduler ID) and the context ID
        ...
        // If cancelled: reload the page (view.php)
        ...
        if ($data = $mform_studentcomment->get_data()) {
            ...
            // Get the draft itemid (Files in the drag-and-drop area are automatically saved as draft even before the form is submitted.)
            ...
            if ($itemid != null) {
                ...
                $data = file_postupdate_standard_filemanager($data, 'attachment', $options, $context, $component, 'studentcomment', $itemid);
            
                // Access the content
                $fs = get_file_storage();
                $files = $fs->get_area_files($contextid, 'mod_scheduler', 'studentcomment', $itemid, $sort = "itemid, filepath, filename", $includedirs = true, $updatedsince = 0);
                ...
                if ($files != null) {
                        ...
                        foreach($files as $file){
                            ...
                            $file->get_contextid(),
                            $file->get_component(), // mod_scheduler
                            $file->get_filearea(),  // studentcomment
                            $file->get_itemid(),
                            $file->get_filepath(),  // a single /
                            $file->get_filename(),  // e.g. Desert.jpg
                            false // $forcedownload
                            );
                            $url[]=$tmp;
                        }
                    $_SESSION['fileUrl'] = $url;
                }
                // tell studentview.controller.php that the form data have been collected and are ready for saving
                $_SESSION['okay'] = true;
                // reload view.php (It will at some point include studentview.controller.php)
                redirect($booker->actionurl);
            }
        }
        
        // If the form was neither cancelled nor submitted: Display it within a modal message window.
        echo "<div id=\"student_comment_window\" class=\"modal\">
                <div class=\"modal-content\">
                    <div class=\"modal-header\">
                        <span class=\"close\" onclick=\"closeCommentField();\">&times;</span>
                        <h2>" . get_string('modalheadlineforstudentcomment','scheduler') . "</h2>
                    </div>
        ...
        // function opens modal message window onclick    
        echo '<script type="text/javascript" src="javascript\openStudentComment.js"></script>';
        
        /*********************** Create and print table of bookable slots *****************************/
        ...
        foreach ($booker->slots as $slot) {
            ...
            $teachernote = $this->format_notes($slot->notes, $slot->notesformat, $booker->scheduler->get_context(),'slotnote', $slot->slotid);
            ...
            if ($slot->canbook) {
                ...
                if ( $slot->otherstudents->scheduler->allows_student_comment() ){
                    $buttonID = '' . $slot->slotid;
                    $commentButton = '<button id ="' . $buttonID. '"onclick="openCommentField(' . $buttonID . ');">' . get_string('studentcomment','scheduler') . '</button>';
                    $rowdata[] = $commentButton;
