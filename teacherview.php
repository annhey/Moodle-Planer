...
/************************************ Schedule students automatically ****************************************/

include($CFG->dirroot.'/mod/scheduler/autoschedule.php');

if ($action === 'autoschedule') {    
    
    // 1. Get all slots that are still available for booking in this scheduler
    $scheduler->free_late_unused_slots();
    $availableslots = $scheduler->get_slots_without_appointment($teacherid = 0); // specify teacher or not?
    
    // 2. Get all students who can book but haven't done so, yet
    $studentsToBook = getStudentsToBook($scheduler);
    $studentsToBookID = getIds($studentsToBook);
    
    // 3. Autoschedule all students who have yet to make an appointment
    if (readyForAutobook($studentsToBook, $availableslots)) {
        
        $count = 0;
        foreach ($studentsToBookID as $id) {
            $slot = $availableslots[$count];
            $newapp = new scheduler_appointment($slot);
            $newapp->studentid = $id;
            $newapp->save();
            $count++;
            // Notify student
...
if ($students === 0) {
    ...
    // Add link for autoscheduling students who have yet to make an appointment
    $scheduler->free_late_unused_slots();
    $availableslots = $scheduler->get_slots_without_appointment($teacherid = 0);
    $studentsToBook = getStudentsToBook($scheduler);
    
    if (readyForAutobook($studentsToBook, $availableslots)) {
        $autoschedule = new moodle_url($actionurl, array('what' => 'autoschedule'));
        $maildisplay .= ' &mdash; ';
        $maildisplay .= html_writer::link($autoschedule, get_string('autoschedule', 'scheduler'));
    }
