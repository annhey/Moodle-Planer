 ...
 * @author     2013 Henning Bostelmann and others (see README.txt)
...
class scheduler_editslot_form extends scheduler_slotform_base {
    ...
    /************************ Ergänzung ***************************************/
    // TODO: Relocate to student view(?)
    protected function definitionStudent() {
    ...
    public function save_slot($slotid, $data) {
        ...
        if ($slot->notes != null && $slot->notes != "") {
            global $USER;
            $noteauthor = $USER->firstname . " " . $USER->lastname;
            $slot->notes = "<b>" . $noteauthor . "</b>:<br>" . $slot->notes;
        }
...
class scheduler_addsession_form extends scheduler_slotform_base {
    ...
    protected function definition() {
        ...
        // Radio button selection of weekly (default), biweekly or monthly slot repeats
        $radioarray = array();
        $radioarray[] = $mform->createElement('radio', 'rrepeats', '', get_string('weekly', 'scheduler'), 1);
        $radioarray[] = $mform->createElement('radio', 'rrepeats', '', get_string('biweekly', 'scheduler'), 2);
        $radioarray[] = $mform->createElement('radio', 'rrepeats', '', get_string('monthly', 'scheduler'), 4);
        $mform->addGroup($radioarray, 'radioar', get_string('repeats', 'scheduler'), array(' '), false);
        $mform->setDefault('rrepeats', 1);
