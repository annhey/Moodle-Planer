...
function xmldb_scheduler_upgrade($oldversion=0) {
    ...
    //<FIELD NAME="studentcomment" TYPE="text" NOTNULL="false" SEQUENCE="false" COMMENT="Notes visible to teacher and student"/>
    ...
    /* ************** Enable booking with message form ******************* */
    
    if($oldversion < 2017050400) {
        
        //Add new field in scheduler_appoinments: studentcomment (for messages and file links)
        $table = new xmldb_table('scheduler_appointment');
        $studentcomment = new xmldb_field('studentcomment', XMLDB_TYPE_TEXT, null, null, null, null, null, 'appointmentnoteformat');
        if (!$dbman->field_exists($table, $studentcomment)) {
            $dbman->add_field($table, $studentcomment);
        }
        //new fields:allowstudentcomment in scheduler and studentcomment in appoinments
        $table = new xmldb_table('scheduler');
         $alcomment = new xmldb_field('allowstudentcomment', XMLDB_TYPE_INTEGER, '4', XMLDB_UNSIGNED,
            XMLDB_NOTNULL, null, '0', 'allownotifications');
        if (!$dbman->field_exists($table, $alcomment)) {
            $dbman->add_field($table, $alcomment);
        }
        upgrade_mod_savepoint(true, 2017050400, 'scheduler');
    }
